...
class GameBoard extends Component {
  ...
  handleResize(e) {
    this.setState({ size: Math.min(window.innerWidth, window.innerHeight) });
  }
  ...
  render() {
    ...
    let tiles = this.props.tiles.map((tile, index) => {
      return <GameTile size={tileSize} onClick={() => { this.props.onSelectTileClick(tile) }} tile={tile} key={index} />
    })
